...
def test_comparison_contract_and_original_experiment_equivalence(datasets):
    ...
    assert table.rows.tolist() == [len(evaluation)] * 3
    assert table.positives.tolist() == [8] * 3
    assert table.negatives.tolist() == [24] * 3
    assert table.prevalence.tolist() == [0.25] * 3
    ...
    assert len(report["model_local_horizon_metrics"]) == 72
...
def test_identical_inputs_labels_and_train_only_fit(datasets, monkeypatch):
    ...
    for cls in (IsolationForest, StandardScaler, OneClassSVM, LocalOutlierFactor):
        guard_fit(cls)
    ...
    assert len(evaluated) == len(scored_inputs) == 3
    assert all(features is scored_inputs[0] for features in scored_inputs)
    assert fit_calls.count(StandardScaler) == 2
    for cls in (IsolationForest, OneClassSVM, LocalOutlierFactor):
...
def test_save_comparison_csv_contents(datasets, tmp_path):
    ...
    for key, path in report["output_files"].items():
        pd.testing.assert_frame_equal(pd.read_csv(path), report[key])
